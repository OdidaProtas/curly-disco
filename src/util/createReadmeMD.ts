...
export default (root: string, opts: IPackageJSON) => {
  ...
  return new Promise((resolve): void => {
    ...
    fs.writeFileSync(
      tsconfigPath,
      content(name, githubUsername, description),
      (err: any) => {
...
[![Node.js CI](https://github.com/${githubUsername}/${name}/actions/workflows/node.js.yml/badge.svg)](https://github.com/${githubUsername}/${name}/actions/workflows/node.js.yml)
[![codecov](https://codecov.io/gh/${githubUsername}/${name}/branch/master/graph/badge.svg?token=UYJAL9KTMD)](https://codecov.io/gh/${githubUsername}/${name})
...
\`\`\`bash
npx create-sky-service <project-name>
\`\`\`

Or

\`\`\`bash
npm init sky-service <project-name>
\`\`\`
...
\`\`\`bash
git clone --depth 1 https://github.com/${githubUsername}/${name}.git
cd ${name}
\`\`\`
